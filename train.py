...
def train():
    ...
    for epoch in range(cfg.epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{cfg.epochs} [Train]"):
            ...
            align_l = mse_loss(F_aligned, text_pooled)
            ...
            total_loss = cfg.lambda_align*align_l +  cfg.lambda_contr*contr_l + cfg.lambda_lm*lm_l
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(tqdm(val_loader, desc=f"Epoch {epoch+1}/{cfg.epochs} [Val]")):
                ...
                if batch_idx == 0 and epoch % 3 == 0:
                    img_norm = F_aligned
                    txt_norm = text_pooled
                    sim_matrix = img_norm @ txt_norm.T  # shape [B, B]

                    plt.figure(figsize=(6,5))
                    plt.imshow(sim_matrix.detach().cpu().numpy(), cmap='plasma', vmin=-1, vmax=1)
                ...
                total_loss = 1.0*align_l +  0.5*contr_l + 0.5*lm_l
        ...
        print(f"Train Align: {align_train/Ntrain:.4f} | Train Contrastive: {contr_train/Ntrain:.4f} | Train LM: {lm_train/Ntrain:.4f}")
        print(f"Val Align: {align_val/Nval:.4f} | Val Contrastive: {contr_val/Nval:.4f} | Val LM: {lm_val/Nval:.4f}")
